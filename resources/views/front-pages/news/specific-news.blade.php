                    ...
                    <div class="full">
                        <div class="title-holder">
                            <div class="title-holder-cell text-left">
                               @if($news->activity_type != 'scholarship')
                                    <h1 class="page-title">{{ $news->body ? 'Udasa Update' : 'Udasa Newsletter'}}</h1>
                                @else
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-9" style="background-color: aliceblue;">
                    <div class="p-3">
                        <h2 class="title" style="color: #029EE3;">{{$news->title}}</h2>
                        <div class="news-body"> {!!$news->body ?? $news->description !!} </div>
                        <div class="pull-right"> 
                            @if($news->file != '')
                                <a class="text-primary" target="_blank" href="/{{ $news->file}}">{{ $news->body ? '' : 'View full PDF file here'}} </a>
                            @else
                                <!-- No any file -->
                            @endif
                        </div>
                ...
                <div class="col-md-3">
                    <div class="side_bar">
                        ...
                        <div class="side_bar_blog">
                            ...
                            <div class="side_bar_blog">
                                <h4  class="color">RECENT {{ $news->body ? 'NEWSLETTER' : 'UPDATES'}}</h4>
                                <div class="categary">
                                    <ul>
                                        @if($recents->IsNotEmpty())
                                            @foreach($recents as $recent)
                                                
                                                <li><a href="{{ $news->file ? url('read-newsletter', $recent->id) : url('read-news', $recent->id)}}"><i class="fa fa-angle-right"></i> {{$recent->title}}</a></li>
                                            @endforeach
                                        @endif
                                    </ul>
                                </div>
